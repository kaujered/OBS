...
use crate::modes::ppl;
use crate::ui::app::{
    TaskState, default_selected_mests, default_year, finish_with_paths, spawn_result_task,
    year_options,
};
...
impl PplScreen {
    ...
    pub(crate) fn from_cli(cli: &Cli) -> Self {
        let (year, _month, _day) = cli.parsed_date();
    ...
    pub(crate) fn render(&mut self, ui: &mut egui::Ui, scale: &LayoutScale, palette: Palette) {
        ...
        ui.painter().line_segment(
            [scale.point(402.0, 472.0), scale.point(646.0, 472.0)],
            Stroke::new(scale.px(1.0), palette.border),
        );
        ...
        if put_button(
            ui,
            scale.rect(RectSpec::new(379.0, 518.0, 290.0, 35.0)),
            scale,
            "Только последняя статика",
            if self.only_last {
        ...
        if put_button(
            ui,
            scale.rect(RectSpec::new(377.0, 567.0, 290.0, 35.0)),
            scale,
        ...
        if put_button(
            ui,
            scale.rect(RectSpec::new(377.0, 615.0, 290.0, 35.0)),
            scale,
            "Debug",
            if self.debug_mode {
                ButtonVisuals::active(palette)
    ...
    /// Start the PPL export task.
    fn start_task(&mut self) {
        let request = ppl::Request {
        ...
        self.task.start(rx, "Запущено формирование статики...");
        spawn_result_task(tx, "Статика", move || ppl::execute(&request));
    }

    fn poll(&mut self) {
        self.task.update_with(finish_with_paths, || {
            "Фоновая задача статики была прервана.".to_string()
        });
    }
