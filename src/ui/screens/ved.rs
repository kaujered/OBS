...
impl VedScreen {
    ...
    pub(crate) fn from_cli(cli: &Cli) -> Self {
        ...
        Self {
            ...
            correct_work_params: !cli.no_correct,
            task: TaskState::default(),
            pending_start: true,
        }
    }
    ...
    pub(crate) fn render(&mut self, ui: &mut egui::Ui, scale: &LayoutScale, palette: Palette) {
        ...
        if put_button(
            ui,
            scale.rect(RectSpec::new(377.0, 569.0, 290.0, 35.0)),
            scale,
        ...
        if put_button(
            ...
            scale,
            "Корректировать рабочие параметры",
            if self.correct_work_params {
                ButtonVisuals::active(palette)
            } else {
                ButtonVisuals::neutral(palette)
            },
        ...
        .clicked()
        {
            self.correct_work_params = !self.correct_work_params;
        }
    ...
    /// Start the Ведомость export task.
    fn start_task(&mut self) {
        let request = ved::Request {
            mests: self.selected.iter().copied().collect(),
            year: self.year,
            month: self.month,
            test_mode: self.test_mode,
            debug_mode: self.debug_mode,
            correct_work_params: self.correct_work_params,
        };
        let (tx, rx) = mpsc::channel();
        self.task.start(rx, "Запущено формирование ведомости...");
        spawn_result_task(tx, "Ведомость", move || ved::execute(&request));
    }

    fn poll(&mut self) {
        self.task.update_with(finish_with_paths, || {
            "Фоновая задача ведомости была прервана.".to_string()
        });
    }
